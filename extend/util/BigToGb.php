...
class BigToGb
{
     ...
     * @return    mixed
     */
    public static function gbToBig($str)
    {
        $str_t = '';
        $len = strlen($str);
        $a = 0;
     ...
     * @return    string
     */
    public static function bigToGb($str): string
    {
        $str_t = '';
        $len = strlen($str);
        $a = 0;
        while ($a < $len) {
            if (ord($str[$a]) >= 224 && ord($str[$a]) <= 239 &&
                ($temp = strpos(static::$utf8_big5, $str[$a] . $str[$a + 1] . $str[$a + 2])) !== false) {
                $str_t .= static::$utf8_gb2312[$temp] . static::$utf8_gb2312[$temp + 1] . static::$utf8_gb2312[$temp + 2];
                $a += 3;
                continue;
            }
            $str_t .= $str[$a];
