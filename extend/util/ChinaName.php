...
class ChinaName
{
    private static $arrXing, $numbXing;
    private static $arrMing, $numbMing;

    public static function rndChinaName()
    {
        self::getXingList();
        self::getMingList();
    }
    ...
    获取姓列表
    */
    private static function getXingList()
    {
        self::$arrXing = explode(
            ',',
    ...
    获取名列表
    */
    private static function getMingList()
    {
        self::$arrMing = explode(
            ',',
        ...
        self::$numbMing = count(self::$arrMing); //名总数
    }

    private static function getXing()
    {
        return self::$arrXing [mt_rand(0, self::$numbXing)];
    }

    private static function getMing()
    {
        return self::$arrMing [mt_rand(0, self::$numbMing)];
    }

    /**
     * @param int $type 1=1姓+1名,2=1姓+1/2名,3=姓,4=名,0=1姓+2名
     * @return string
     */
    public static function getName($type = 0)
    {
        ...
        switch ($type) {
            ...
            case 2: //随机2、3个字
                $name = self::getXing() . self::getMing();
                if (mt_rand(0, 100) > 50) $name .= self::getMing();
                break;
            case 3: //只取姓
                $name = self::getXing();
                break;
    ...
    public static function getNickname(): string
    {
        ...
        $tou_num = random_int(0, 331);
        $do_num = random_int(0, 19);
        $wei_num = random_int(0, 327);
        $type = random_int(0, 1);
        if ($type == 0) {
    ...
    public static function getNicknameByLocalSend($lang = 'zh'): string
    {
        $adjectives = [
            'zh' => ['迷人', '美丽', '巨大', '明亮', '干净', '聪明', '帅气', '可爱', '狡猾', '坚定', '有活力', '高效', '极好', '快速', '不错', '新鲜', '好', '华丽', '伟大', '英俊', '炽热', '善良', '诚实', '神秘', '整洁', '开心', '耐心', '漂亮', '强大', '富有', '秘密', '聪明', '稳固', '特别', '战略性', '强大', '整洁', '智慧',],
            'en' => ['Adorable', 'Beautiful', 'Big', 'Bright', 'Clean', 'Clever', 'Cool', 'Cute', 'Cunning', 'Determined', 'Energetic', 'Efficient', 'Fantastic', 'Fast', 'Fine', 'Fresh', 'Good', 'Gorgeous', 'Great', 'Handsome', 'Hot', 'Kind', 'Lovely', 'Mystic', 'Neat', 'Nice', 'Patient', 'Pretty', 'Powerful', 'Rich', 'Secret', 'Smart', 'Solid', 'Special', 'Strategic', 'Strong', 'Tidy', 'Wise',]
        ];
        $ruits = [
            'zh' => ['苹果', '鳄梨', '香蕉', '黑莓', '蓝莓', '西兰花', '胡萝卜', '樱桃', '椰子', '葡萄', '柠檬', '莴苣', '芒果', '甜瓜', '蘑菇', '洋葱', '橙子', '木瓜', '桃子', '梨', '菠萝', '土豆', '南瓜', '覆盆子', '草莓', '番茄',],
            'en' => ['Apple', 'Avocado', 'Banana', 'Blackberry', 'Blueberry', 'Broccoli', 'Carrot', 'Cherry', 'Coconut', 'Grape', 'Lemon', 'Lettuce', 'Mango', 'Melon', 'Mushroom', 'Onion', 'Orange', 'Papaya', 'Peach', 'Pear', 'Pineapple', 'Potato', 'Pumpkin', 'Raspberry', 'Strawberry', 'Tomato',],
        ];
        $first = $adjectives[$lang];
        $second = $ruits[$lang];
        $middle = $lang == 'zh' ? '的' : ' ';

        return $first[array_rand($first)] . $middle . $second[array_rand($second)];
    }
}
