 ...
 * @icon fa fa-circle-o
 */
class SysUserRole extends Backend
{

    /**
     * UserShutup模型对象
     * @var \app\admin\model\SysUserRole
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\SysUserRole();
    }
    ...
    public function index()
    {
        ...
        foreach ($list->items() as $k => $v) {
            ...
            $list->items()[$k]['operator_nickname'] = RedisService::getAdminCache($v['audit_admin'], 'nickname');
        }
    ...
    public function add()
    {
        ...
        try {
            ...
            $exist = Db::name('user')->where('id',$params['user_id'])->value('id');
            if (!$exist) {
                throw new Exception('用户ID不正确');
            }
            $role = Db::name('user_business')->where('id',$params['user_id'])->value('role');
            if ($role != 1) {
                throw new Exception('当前身份不可设置为运营');
            }
