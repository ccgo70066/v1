 ...
 * @icon fa fa-circle-o
 */
class BusinessLogH extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $total = $this->model
                ->with($filter ? $with : [])
                ->where($where)
                ->where('user_id',redis()->get($this->request->controller().$this->request->action(). $this->auth->id))
                ->count();
            ...
            redis()->set($this->request->controller().$this->request->action(). $this->auth->id,input('user_id'),10);
        }
