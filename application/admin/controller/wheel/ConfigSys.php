...
{
    // php think crud -t wheel_config_sys -m wheelConfigSys -c wheel/configSys -u 1 -f 1
    /**
     * WheelConfigSys模型对象
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\WheelConfigSys;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $key = 'pool:wheel:sys_' . input('box_type');
            $gift = [
                'clear_type' => 1,
                'box_type'   => input('box_type'),
                'count_type' => input('count_type'),
            ];
            $gift_ids = db('wheel_clear_gift')->where($gift)->value('gift_ids');
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    isset($params['title']) && $params['title'] = trim($params['title']);
    ...
    public function set($type, $box_type)
    {
        ...
        if ($type == 2) {
            ...
            if ($this->request->isPost()) {
                ...
                db('wheel_clear_gift')->insert($gift, true);
                $this->success();
            }
            return $this->view->fetch('set1', ['gift_ids' => db('wheel_clear_gift')->where($gift)->value('gift_ids')]);
        }
    }
