...
class LogMongo extends Backend
{
    ...
    /**
     * WheelLog模型对象
     * @var \app\admin\model\WheelLogMongo
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\WheelLogMongo;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as &$item) {
                ...
                $item['gift_price'] = $gift[$item['log']['gift_id']]['price'];
        ...
        return $this->view->fetch('wheel/log/index');
    }
    ...
    public function sum()
    {
        ...
        $sum = $this->model->command($command)[0] ?? 0;
        $total_used_amount = $sum['used_amount'];
        $total_gain_amount = $sum['amount'];
        $result = [
            'total_used_amount' => number_format($total_used_amount / 1000, 5),
            'total_gian_amount' => number_format($total_gain_amount / 1000, 5),
            'total_our_amount'  => number_format(($total_gain_amount * 0.85 - $total_used_amount) / 10 / 1000, 5),
