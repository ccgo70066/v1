...
class ConfigPubn extends Backend
{
    ...
    /**
     * WheelConfigPubn模型对象
     * @var \app\admin\model\WheelConfigPubn
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\WheelConfigPubn;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $key = 'pool:wheel:pubn_' . input('box_type');
    ...
    {
        $key = 'pool:wheel:pubn_' . $box_type;
        if ($this->request->isPost()) {
            Cache::inc($key, input('row.diff') * 100);
            $this->success('');
        }
        return $this->view->fetch('', [
            'value' => (100000 * 100 - cache($key)) / 100,
        ]);
    }
}
