 ...
 * @icon fa fa-circle-o
 */
class RoomThemeCate extends Backend
{

    /**
     * RoomThemeCate模型对象
     * @var \app\admin\model\RoomThemeCate
     */
    ...
    public function get_top_cate()
    {
        if ($this->request->request("keyValue")) {
            return ['total' => 1, 'list' => [
                db('room_theme_cate')
                    ->where(['id' => $this->request->request("keyValue")])
                    ->field('name,id')
                    ->find()
            ]];
        }

        $sel = db('room_theme_cate')
            ->where([ 'status' => 1])
            ->field('name,id')
            ->select();
        array_unshift($sel, ['id' => 0, 'name' => '顶级分类']);
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                (mb_strlen($params['intro']) > 20) && $this->error('描述长度必须在20字以内');
