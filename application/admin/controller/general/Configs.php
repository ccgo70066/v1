 ...
 *
 * @icon   fa fa-cogs
 * @remark 可以在此修改运营相关的变量,请谨慎操作, 修改前请务必熟悉变量修改后对运营的影响!
 */
class Configs extends Backend
{
    ...
    {
        $siteList = [];
        $groupList = json_decode(get_site_config('app_config_group'), true);
...
//                        ConfigModel::refreshFile();
    ...
    public function edit($ids = null)
    {
        if ($this->request->isPost()) {
            ...
            if ($row) {
                ...
                if (isset($row['base_version']) || isset($row['show_level'])) {
                    //$fileName = 'n.htm';
                    //file_put_contents($fileName, $this->getConfigString());
                }
                $this->success();
            }
            $this->error(__('Parameter %s can not be empty', ''));
    ...
    public function getConfigString()
    {
        ...
        return $des->encrypt(json_encode($config));
    }
...
//                ConfigModel::refreshFile();
    ...
    public function emailtest()
    {
        ...
        if ($receiver) {
            ...
            $result = $email
                ...
                ->message(
                    '<div style="min-height:550px; padding: 100px 55px 200px;">' . __(
    ...
    public function selectpage()
    {
        ...
        $this->request->request([
            'showField'   => $setting['field'],
            'keyField'    => $setting['primarykey'],
            'custom'      => $custom,
            'searchField' => [$setting['field'], $setting['primarykey']]
        ]);
