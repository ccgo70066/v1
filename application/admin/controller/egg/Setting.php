            ...
            if ($row) {
                ...
                foreach ($this->model->all() as $v) {
                    if (isset($row[$v['name']])) {
                        ...
                        cache('config:' . $v['name'], null);
                        // 更新奖池
                        // if(in_array($v['name'], ['box_pool_type1', 'box_pool_type2', 'box_pool_type3'])){
                        //     cache('site.' . $v['name'], $value);
                        // }
                    }
                }
                try{
                    $this->model->allowField(true)->saveAll($configList);
                    Cache::clear('small_data_fish');
                }catch (Exception $e){
                    $this->error($e->getMessage());
                }
                try{
