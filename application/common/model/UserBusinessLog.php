...
class UserBusinessLog extends Model
{
    ...
    //数值类型:1=积分,2=金幣,3=红豆,4=收益,5=能量
    const TYPE_INTEGRAL = 1; //积分
    const TYPE_DIAMOND = 2; //金幣
    const TYPE_RED_BEANS = 3; //红豆
    const TYPE_INCOME = 4; //收益
    const TYPE_FRAGMENT = 5; //能量

    //分类:0=其它,1=商城兑换,2=活动奖励,3=充值金幣,4=打赏礼物,5=赠送盲盒,6=IM红包,7=房间红包,8=兑换金幣,9=兑换游戏券,10=守护分成,11=家族收益提领,12=流水奖励,13=收益提现
    const FROM_MALL_EXCHANGE = 1; //商城兑换
    const FROM_TASK = 2; //活动奖励
    const FROM_RECHARGE = 3; //充值金幣
    const FROM_REWARD = 4; //打赏礼物
    const FROM_SEND_BOX = 5; //赠送盲盒
    const FROM_IM_LUCKY_MONEY = 6; //IM红包
    const FROM_ROOM_LUCKY_MONEY = 7; //房间红包
    const FROM_CHANGE_DIAMOND = 8; //兑换金幣
