...
class Member extends Base
{
    ...
    public function get_list()
    {
        ...
        $list = db('room_admin')->alias('a')->join('user u', 'a.user_id = u.id', 'left')
            ->field('a.user_id,a.role,a.status,u.id,u.nickname,u.avatar,u.birthday,gender,level,a.create_time,a.update_time' . $extend)
    ...
    public function dismiss()
    {
        ...
        if (!$role) $this->error(__('You have no permission'));
        $member = db('room_admin')->where(['room_id' => $room_id, 'status' => 1, 'role' => ['in', '2,3']])->count();
        if ($member) $this->error(__('There are still members in the room'));
