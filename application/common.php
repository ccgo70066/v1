    ...
    function check_cors_request()
    {
        if (isset($_SERVER['HTTP_ORIGIN']) && $_SERVER['HTTP_ORIGIN'] && config('fastadmin.cors_request_domain')) {
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
                ...
                header("Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With, lang, token");
                header("Access-Control-Max-Age: 86400"); // 預檢結果快取 24 小時
                header("Access-Control-Allow-Credentials: true"); // 如果未來要用 cookie

                // 必須返回 204 No Content（或 200）
