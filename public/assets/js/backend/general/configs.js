    ...
    var Controller = {
        index: function () {
            ...
            $(document).on('change', "#c-selectpage-table", function (e, first) {
                var that = this;
                Fast.api.ajax({
                    url: "general/configs/get_fields_list",
                    data: {table: $(that).val()},
                }, function (data, ret) {
            ...
            $(document).on("change", "#c-type", function () {
                ...
                if (["selectpage", "selectpages"].indexOf(value) > -1 && $("#c-selectpage-table option").length == 1) {
                    //异步加载表列表
                    Fast.api.ajax({
                        url: "general/configs/get_table_list",
                    }, function (data, ret) {
            ...
            //添加向发件人发送测试邮件按钮和方法
            $('input[name="row[base_version]"]').parent().next().append('<a class="btn btn-info push_config">' + __('推送基础配置') + '</a>');
            $(document).on("click", ".push_config", function () {
                var that = this;
                Layer.confirm(__('您确认要推送基础配置到前端吗? 推送前请更新基础配置版本号并点击确定按钮保存!'), {
                    icon: 3,
                    title: '提示'
                }, function (index) {
                    Backend.api.ajax({
                        url: "general/configs/push_config",
