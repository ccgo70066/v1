define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'egg/log_mongo_h/index' + location.search,
                    sum_url: 'egg/log_mongo_h/sum' + location.search,
                    detail_url: 'egg/log_mongo_h/index',
                    table: 'egg_log',
                }
            });
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        // {field: 'actor_status', title: __('Actor_status'), searchList: {"1":__('Actor_status 1'),"2":__('Actor_status 2'),"3":__('Actor_status 3')}, formatter: Table.api.formatter.normal},
                        ...
                        {field: 'log.level_id', title: __('Level_id'), operate: 'false', searchList: $.getJSON('egg/log/index/option/load_level/'), formatter: function (value, row, index) {
                                return row.level_name;
                            }},
                        {field: 'used_amount', title: __('Used_amount'), operate: false, formatter: function (value, row, index) {
                                return value;
                            }},
                        {field: 'log.gift_id', title: __('礼物'), searchList: $.getJSON('egg/log/index/option/load_gift'), formatter: function (value, row, index) {
                                return row.gift_name;
                            }},
                        {field: 'log.gift_value', title: __('礼物价值'), operate: false},
                        // {field: 'count', title: __('数量'), operate: false},
                        {field: 'room_id', title: __('房间号')},
                        {field: 'log.box_index', title: __('序号'), operate: false},
                        {field: 'log.weigh_name', title: __('Weigh_name'), operate: '=', searchList: $.getJSON('egg/log/index/option/load_weigh')},
                        ...
                        {field: 'log.pool_pubn_before', title: __('pool_pubn_before'),  operate: false},
                        {field: 'log.pool_pubn_after', title: __('Pool_pubn_after'),  operate: false},
                        {field: 'log.pool_pubn_diff', title: __('Pool_pubn_diff'),  operate: false},
                        ...
                        {field: 'ip', title: __('登录ip'),  operate: false},
